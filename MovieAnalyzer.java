    ...
    public MovieAnalyzer(String dataset_path){
        ...
        try {
            ...
            movieList = br.lines().skip(1).map(line -> {
                ...
                //movieInfo.Series_Title = data[1].replace("|", "").replace("\"", "");
                //System.out.println(movieInfo.Series_Title);
...
//            MovieInfo mi=movieList.skip(i).findFirst().get();
//            for(int j=0;j<mi.getGenre().size();j++){
//                if(movieCountByGenre.containsKey(mi.getGenre().get(j))){
//                    movieCountByGenre.put(mi.getGenre().get(j),movieCountByGenre.get(mi.getGenre().get(j))+1);
//                }
//                else{
//                    movieCountByGenre.put(mi.getGenre().get(j),1);
//                }
//            }
//        }
        return ans;
...
//                .forEach(entry -> ans.put(entry.getKey(), entry.getValue()));
    ...
    public List<String> searchMovies(String genre, float min_rating, int max_runtime){
        ...
        Stream<MovieInfo> movieListTemp=movieTemp.stream();
        movieListTemp.filter(x->x.getGenre().contains(genre) && x.getIMDB_Rating()>=min_rating && x.getRuntime()<=max_runtime).sorted(Comparator.comparing(x->x.getSeries_Title())).forEach(x->searchMovies.add(x.getSeries_Title()));
        return searchMovies;
    }
    ...
    public static void main(String[] args) {
        ...
        //System.out.println(movieAnalyzer.getTopMovies(10,"gross"));
        System.out.println(movieAnalyzer.getCoStarCount());
        //System.out.println(movieAnalyzer.getTopStars(15, "gross"));
        //System.out.println(movieAnalyzer.searchMovies("Action", 8.0f, 120));
    }
